...
import { SSH_AGENTC_REQUEST_IDENTITIES, SSH_AGENTC_SIGN_REQUEST, SSH_AGENT_IDENTITIES_ANSWER, appendIdentity, computeFingerprint, decodePublicKeyLine, extractMessages, readSignRequest, writeFailure, writeIdentitiesAnswer, writeSignResponse, } from "./ssh-agent-protocol.js";
/**
 ...
 */
function signerIntercept(fingerprint, apiURL, signingKey, displayVerification, standalone, agentkitAuth) {
    // Store bytes in buffer to concat messages arriving in multiple chunks (per direction)
    let requestBuffered = Buffer.alloc(0);
    let responseBuffered = Buffer.alloc(0);
    // Decode once so it can be added to every identity listing
    const signingKeyBlob = decodePublicKeyLine(signingKey);
    // Return interceptor
    return async (data, direction, context) => {
        ...
        if (direction === 'response') {
            ...
            const forwarded = messages.map((message) => {
                const isIdentitiesAnswer = message.length > 4 &&
        ...
        for (const message of messages) {
            ...
            if (type === SSH_AGENTC_SIGN_REQUEST) {
                const { keyBlob, data: dataToSign } = readSignRequest(message.subarray(5));
                const keyFingerprint = computeFingerprint(keyBlob);
                if (keyFingerprint === fingerprint) {
                    await remoteSign(apiURL, fingerprint, dataToSign, context, displayVerification, agentkitAuth);
                }
                else if (standalone) {
            ...
            if (type === SSH_AGENTC_REQUEST_IDENTITIES && standalone) {
                context.respond(writeIdentitiesAnswer([
 ...
 */
async function fetchVerification(apiURL, requestId) {
    ...
    for (let attempt = 0; attempt <= VERIFY_RETRY_ATTEMPTS; attempt += 1) {
        ...
        if (response.ok) {
            ...
            console.warn(`[verify] ${requestId}: attempt ${(attempt + 1).toString()} got ok response with no url, retrying in ${VERIFY_RETRY_DELAY_MS.toString()}ms`);
        }
        else {
            console.warn(`[verify] ${requestId}: attempt ${(attempt + 1).toString()} got ${response.status.toString()}, retrying in ${VERIFY_RETRY_DELAY_MS.toString()}ms`);
        }
    ...
    console.warn(`[verify] ${requestId}: giving up, no verification link retrieved`);
    return null;
}
/**
 ...
 */
async function remoteSign(apiURL, fingerprint, dataToSign, context, displayVerification, agentkitAuth) {
    // Make sign request
    ...
    if (!verification) {
        console.warn(`[sign] ${requestId}: failing, could not retrieve verification link`);
        context.respond(writeFailure());
        return;
    }
    ...
    if (!response.ok) {
        console.warn(`[sign] ${requestId}: rejected or expired (${response.status.toString()})`);
        context.respond(writeFailure());
        return;
    }
    // Else return with successful singature
    console.log(`[sign] ${requestId}: approved`);
    const { format, signature } = (await response.json());
