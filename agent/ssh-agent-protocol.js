 ...
 */
function computeFingerprint(keyBlob) {
 ...
 */
function writeSignResponse(format, signature) {
 ...
 */
function decodePublicKeyLine(line) {
 ...
 */
function appendIdentity(message, keyBlob, comment) {
 ...
 */
function writeIdentitiesAnswer(identities) {
    const count = Buffer.alloc(4);
    count.writeUInt32BE(identities.length, 0);
    const entries = identities.map(({ keyBlob, comment }) => Buffer.concat([
        writeString(keyBlob),
        writeString(Buffer.from(comment, 'utf8')),
    ]));
    const body = Buffer.concat([
...
export { SSH_AGENT_FAILURE, SSH_AGENT_IDENTITIES_ANSWER, SSH_AGENT_SIGN_RESPONSE, SSH_AGENTC_REQUEST_IDENTITIES, SSH_AGENTC_SIGN_REQUEST, appendIdentity, computeFingerprint, decodePublicKeyLine, extractMessages, readSignRequest, readString, writeFailure, writeIdentitiesAnswer, writeSignResponse, writeString, };
