...
function createAgent() {
    ...
    process.stdout.write(`Generated wallet ${address}, saved to ${WALLET_KEY_PATH}\n`);
    process.stdout.write('Registering with AgentBook...\n');
    const result = spawnSync('npx', ['@worldcoin/agentkit-cli', 'register', address], { stdio: 'inherit' });
    if (result.error || result.status !== 0) {
        process.stderr.write(`Registration failed; you can retry with: npx @worldcoin/agentkit-cli register ${address}\n`);
        process.exitCode = 1;
    }
}
export { createAgent };
