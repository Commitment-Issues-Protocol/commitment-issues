 ...
 */
function isAvailable(command, versionFlag) {
 ...
 */
function ensureVsCodeWaits(args) {
 ...
 */
const COMMAND_REWRITES = [ensureVsCodeWaits];
/**
 ...
 */
function applyCommandRewrite(args) {
 ...
 */
function catAndWaitScript(filePath) {
 ...
 */
function launchTmux(sessionName, targetCommand, proxyEnv) {
 ...
 * @param proxyEnv - environment overrides to apply to that command
 */
function launchScreen(sessionName, targetCommand, proxyEnv) {
 ...
 */
function writeVerificationFile(content) {
    const filePath = join(tmpdir(), `commitment-issues-verify-${randomUUID()}.txt`);
 ...
 */
function createPopoverDisplay(multiplexer, sessionName) {
    return async (url) => {
        const qr = await renderQrCode(url);
        const filePath = writeVerificationFile(`${qr}\nVerify you are human: ${url}\n`);
...
function session(args) {
    ...
    if (!multiplexer) {
        process.stderr.write('`session` requires tmux or screen, but neither was found on PATH\n');
        process.exitCode = 1;
        return;
    }
    if (args.length === 0) {
        process.stderr.write('Usage: commitment-issues session <command> [args...]\n');
        process.exitCode = 1;
        return;
    }
    ...
    proxy.intercept = signerIntercept(FINGERPRINT, API_URL, SIGNING_KEY, createPopoverDisplay(multiplexer, sessionName), standalone, agentkitAuth);
    if (standalone) {
